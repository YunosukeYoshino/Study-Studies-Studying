{
  {
    // 135. ドラッグ & ドロップ実装におけるインターフェースの活用
    // Project Type
    enum ProjectStatus {
      Active,
      Finished,
    ...
    function validate(validatableInput: Validatable) {
      ...
      if (
        ...
        isValid =
          isValid &&
          validatableInput.value.length >= validatableInput.minLength; //value値のlengthが最小値以上であるかどうか
      }
      if (
        ...
        isValid =
          isValid &&
          validatableInput.value.length <= validatableInput.maxLength; //value値のlengthが最大値以下であるかどうか
      }
      if (
    ...
    //projectItem Class
    class ProjectItme extends Component<HTMLUListElement, HTMLLIElement> {
